(() => {
  ...
  paintTheme();
  themeBtn.addEventListener('click', () => { Store.setTheme(document.documentElement.dataset.theme === 'dark' ? 'light' : 'dark'); paintTheme(); });
  ...
  $('#scrub-live').addEventListener('click', () => { const n = Math.round(UI.nowMin()); paintDemo(Math.min(1380, Math.max(360, n))); });
  // Start a gentle auto-play until the person touches it
  let auto = true, t = 8 * 60 + 40;
  paintDemo(t);
  const timer = setInterval(() => { if (!auto) return clearInterval(timer); t += 2; if (t > 1300) t = 400; paintDemo(t); }, 80);
  ...
  function mountTasks(el, list) {
    ...
    el.addEventListener('click', e => {
      const row = e.target.closest('.task'); if (!row || !e.target.closest('.check')) return;
  ...
  /* ---------- Feature: mini now (animated progress) ---------- */
  const featNow = $('#feat-now');
  let fp = 20;
  setInterval(() => { fp = fp >= 100 ? 5 : fp + 0.4; $('.bar i', featNow).style.width = fp + '%'; $('.foot span:first-child', featNow).textContent = `${Math.ceil(60 - fp * .6)} min left`; $('.foot span:last-child', featNow).textContent = 'Next: Deep work at 10:30 AM'; }, 80);

  /* ---------- Feature: typing palette ---------- */
  const typed = $('#typed'); const phrases = ['new task', 'insights', 'dark theme', 'log sleep'];
  let pi = 0, ci = 0, del = false;
  setInterval(() => {
    const p = phrases[pi];
    if (!del) { ci++; if (ci > p.length + 12) del = true; }
  ...
  setInterval(() => { $$('#feat-week div').forEach(b => { const v = 1 + Math.random() * 4; b.style.height = v / 5 * 100 + '%'; }); }, 2600);
