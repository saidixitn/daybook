(() => {
  ...
  function renderInsights() {
    ...
    $('#tiles').innerHTML = [
      ...
      { k: 'Sleep average', icon: 'moon', v: avgLabel, d: rated.length ? `${rated.length} nights rated` : 'No logs yet' },
    ...
    /* ---------- Focus bars ---------- */
    const fc = $('#focus-chart');
    const W = 400, H = 150, base = 128, gap = 14, bw = (W - gap * 6) / 7, max = Math.max(6, ...S.focus);
    fc.innerHTML = S.focus.map((v, i) => {
    ...
    /* ---------- Sleep line ---------- */
    const sc = $('#sleep-chart');
    const pad = 44, top = 14, bot = 118;
    ...
    setTimeout(() => {
      const ringEl = $('#ring');
      if (ringEl) ringEl.setAttribute('stroke-dasharray', `${pct * 314} 314`);
    }, 50);
    ...
    if (rated.length >= 3) {
      ...
      if (roughIdx >= 0 && S.focus[roughIdx] < focusTotal / 7) {
        obs.push({ icon: 'moon', text: `A rough night on <b>${UI.DAYS_LONG[roughIdx]}</b> lined up with your lowest focus day. Sleep is showing up in the work.` });
      } else if (avgLabel !== '—') {
        obs.push({ icon: 'moon', text: `Sleep has been mostly <b>${avgLabel.toLowerCase()}</b> this week. That’s a steady base to build on.` });
      }
    } else {
      obs.push({ icon: 'moon', text: `Log your sleep on the Today screen to begin seeing sleep and focus correlations.` });
    }
    obs.push({ icon: 'tasks', text: pct >= .5 ? `You’ve cleared <b>${Math.round(pct * 100)}%</b> of everything on your lists. Keep the momentum going.` : `Most of your tasks are open. Try starring one or two priority items to conquer first.` });
    if (S.focus[worst] < 2) {
      obs.push({ icon: 'calendar', text: `<b>${UI.DAYS_LONG[worst]}</b> has almost no focus time. Block one 90-minute slot before noon to protect it.` });
    }
    $('#insights').innerHTML = obs.map(o => `<div class="insight"><span class="ic">${icon(o.icon)}</span><div>${o.text}</div></div>`).join('');
  }

  document.addEventListener('tasks:changed', renderInsights);
  document.addEventListener('store:synced', renderInsights);
  renderInsights();
})();
